#!/bin/sh

# start-dev.sh
# This script starts the backend and frontend servers for development.

echo "Starting development environment..."

# Step 1: Initialize the database using a Flask CLI command.
# This is a robust way to ensure the tables exist before the API starts.
echo "Waiting for PostgreSQL to be ready..."
sleep 5 # A small safety delay for network DBs to respond on first startup
echo "Initializing database schema via Flask..."
flask --app api/app init-db
status=$?
if [ $status -ne 0 ]; then
  echo "Database initialization failed with status $status. See errors above. Exiting."
  exit $status
fi
echo "Database initialization successful."

# Step 2: Start the Flask backend API in the background
echo "Starting Flask backend server on port 5001..."
flask --app api/app run --host=0.0.0.0 --port=5001 &

# Step 3: Start the React frontend development server in the foreground
echo "Starting React frontend server on port 3000..."
npm start --prefix frontend

